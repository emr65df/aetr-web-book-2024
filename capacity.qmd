...
statewide_pdf <- 
  ...
  ggtitle("Figure 3.1: Capacity Changes, Statewide")
...
```{r}
coastal_names <- c("Fossil Turbines", "Recip Engines", "Hydro", "Wind", "Storage")

coastal_capacity <- 
  regional_capacity %>%
  filter(acep_region == "Coastal", prime_mover %in% coastal_names) %>%
  group_by(year, prime_mover) %>%
      summarize(coastal_capacity = sum(total_capacity, na.rm = TRUE), .groups = "drop") %>% 
      mutate(prime_mover = factor(prime_mover, 
                                  levels = rev(c("Fossil Turbines", "Recip Engines", "Hydro", "Wind", "Storage"))),
             year = as.character(year))
...
coastal_plot <-
  ...
  scale_fill_manual(values = rev(c("#606571", "#9da7bf", "#00a1b7", "#F79646","#71346a"))) +
...
coastal_pdf <- 
  ...
  ggtitle("Figure 3.2: Coastal Region Capacity")
...
coastal_renewable_capacity <- 
  ...
  group_by(year, prime_mover) %>%
      ...
      mutate(prime_mover = factor(prime_mover, 
                                  levels = rev(c("Hydro", "Wind", "Storage"))),
             year = as.character(year))
...
coastal_renewable_plot <-
  ...
  scale_fill_manual(values = rev(c("#00a1b7", "#F79646","#71346a"))) +
...
coastal_renewable_pdf <- 
  ...
  ggtitle("Figure 3.3: Coastal Region Renewable Capacity")
...
railbelt_capacity <- 
  ...
  group_by(year, prime_mover) %>%
      ...
      mutate(prime_mover = factor(prime_mover, 
                                  levels = rev(c("Fossil", "Turbines","Recip Engines","Hydro","Wind","Utility Solar","Rooftop Solar","Storage","Landfill", "Gas"))),
             year = as.character(year))
...
railbelt_plot <-
  ...
  scale_fill_manual(values = rev(c("#606571", "#9da7bf", "#00a1b7", "#F79646", "#fad900","#9BBB59","#71346a","#896D09"))) +
...
railbelt_pdf <- 
  ...
  ggtitle("Figure 3.4: Railbelt Region Capacity")
...
railbelt_renewables_names <- c("Hydro", "Wind", "Utility Solar", "Rooftop Solar", "Storage", "Landfill Gas")

railbelt_renewable_capacity <- 
  regional_capacity %>%
  filter(acep_region == "Railbelt", prime_mover %in% railbelt_renewables_names) %>%
  group_by(year, prime_mover) %>%
...
railbelt_renewable_plot <-
  ...
  scale_fill_manual(values = rev(c("#00a1b7", "#F79646","#fad900","#9BBB59","#71346a","#896D09"))) +
...
railbelt_renewable_pdf <- 
  ...
  ggtitle("Figure 3.5: Railbelt Region Renewable Capacity")
...
rural_remote_plot <-
  ...
  scale_fill_manual(values = rev(c("#606571", "#9da7bf", "#00a1b7", "#F79646", "#fad900","#71346a"))) +
...
rural_remote_pdf <- 
  ...
  ggtitle("Figure 3.6: Rural Remote Region Capacity")
...
rural_remote_renewables_names <- c("Hydro", "Wind", "Utility Solar","Storage")

rural_remote_renewable_capacity <- 
  regional_capacity %>%
  filter(acep_region == "Rural Remote", prime_mover %in% rural_remote_renewables_names) %>%
  group_by(year, prime_mover) %>%
...
rural_remote_renewable_plot <-
  ...
  scale_fill_manual(values = rev(c("#00a1b7", "#F79646","#fad900","#71346a"))) +
...
rural_remote_renewable_pdf <- 
  ...
  ggtitle("Figure 3.7: Railbelt Region Renewable Capacity")
...
We begin this section by showcasing the increases in total capacity across the state. In 2011, it is estimated that the total statewide electricity generation capacity was `r capacity(year=2011)` MW. We estimate that this has increased to approximately `r capacity(year=2021)` MW in 2021 based on best available data. This represents an increase of `r formatC(capacity(year=2021, numeric_out = TRUE) - capacity(year=2011, numeric_out = TRUE), format="d", big.mark=",")` MW, or `r round((capacity(year=2021, numeric_out = TRUE) - capacity(year=2011, numeric_out = TRUE))/abs(capacity(year=2011, numeric_out = TRUE)),2)*100` percent increase since 2011. To illustrate this example, we show a stacked area chart in @fig-capacity-state that showcases growth over time for various technologies.[^2]

[^2]: Prime movers are categorized as follows. Fossil turbines include combined cycle turbines, gas turbines, and steam turbines. Reciprocating engines include internal combustion engines. Hydro includes hydraulic turbines and hydrokinetics. Wind includes wind turbines. Utility solar includes utility-owned photovoltaic (PV), and Rooftop solar includes customer-sited, behind-the-meter PV. Storage refers to batteries and flywheels.

```{r}
#code for both html and pdf

names <- c("Landfill Gas","Storage","Rooftop Solar","Utility Solar","Wind","Hydro","Recip Engines","Fossil Turbines")
