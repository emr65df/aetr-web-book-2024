...
regional_generation_data <- generation_data %>%
  ...
  mutate(acep_region = factor(acep_region, levels = rev(c("Coastal", "Railbelt", "Rural Remote"))))
...
regional_generation_plot <-
  ...
  scale_fill_manual(values = rev(c("#8cbbda","#97cd93","#f28d8c"))) +
...
gen_mix_diff <- join_shares %>%
  filter(year %in% c(2011, 2021)) %>%
  mutate(delta = generation_share - lag(generation_share, n = 21)) %>%
  filter(year == 2021)
